...
##########################################################################
# This script is used to deploy the AWS infrastructre for the Prod Cluster
...
PREFIX="kube-demo-prod-" #Add default for the prefix used with the names of all resources to be kube-demo-
...
MNODE1A_IP="10.11.11.100" # Add default IP address for Master Node
WNODE1A_NAME="${PREFIX}01-worker-01" # Add default name for Worker Node 1
WNODE1A_IP="10.11.11.101" # Add default IP address for worker Node 1
WNODE1B_NAME="${PREFIX}01-worker-02" # Add default name for Worker Node 2
WNODE1B_IP="10.11.11.102" # Add default IP address for Worker Node 2
...
VPC_ID=$(aws ec2 create-vpc --region $REGION --cidr-block '10.11.0.0/16' --output json | jq '.Vpc.VpcId'| sed -e 's/^"//' -e 's/"$//') #Create VPC
...
SUBNET1_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --region $REGION --cidr-block 10.11.10.0/24 --availability-zone $AZ1 --output json | jq '.Subnet.SubnetId'| sed -e 's/^"//' -e 's/"$//')
...
SUBNET1A_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --region $REGION --cidr-block 10.11.11.0/24 --availability-zone $AZ1 --output json | jq '.Subnet.SubnetId'| sed -e 's/^"//' -e 's/"$//')
...
NULL=$(aws ec2 authorize-security-group-ingress --region $REGION --group-id $PUBSGROUP_ID --protocol all --cidr 10.11.0.0/16) #Add Rules to Security Group.
        #-------------------------------------------------------------------------
